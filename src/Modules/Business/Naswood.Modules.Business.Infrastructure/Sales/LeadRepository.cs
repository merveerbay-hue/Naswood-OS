...
public sealed class LeadRepository : ILeadRepository
{
    ...
    public LeadRepository(BusinessDbContext db) => _db = db;

    public Task<Lead?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default) =>
        _db.Set<Lead>().FirstOrDefaultAsync(x => x.Id == id, cancellationToken);

    public async Task AddAsync(Lead entity, CancellationToken cancellationToken = default) =>
        await _db.Set<Lead>().AddAsync(entity, cancellationToken).ConfigureAwait(false);

    public async Task<(IReadOnlyList<Lead> Items, int Total)> SearchAsync(string? q, int page, int pageSize, CancellationToken cancellationToken = default)
    {
        var query = _db.Set<Lead>().AsNoTracking().Where(x => !x.IsDeleted);
