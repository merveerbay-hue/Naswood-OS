...
public sealed class PurchaseOrderRepository : IPurchaseOrderRepository
{
    ...
    public PurchaseOrderRepository(BusinessDbContext db) => _db = db;

    public Task<PurchaseOrder?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default) =>
        _db.Set<PurchaseOrder>().FirstOrDefaultAsync(x => x.Id == id, cancellationToken);

    public async Task AddAsync(PurchaseOrder entity, CancellationToken cancellationToken = default) =>
        await _db.Set<PurchaseOrder>().AddAsync(entity, cancellationToken).ConfigureAwait(false);

    public async Task<(IReadOnlyList<PurchaseOrder> Items, int Total)> SearchAsync(string? q, int page, int pageSize, CancellationToken cancellationToken = default)
    {
        var query = _db.Set<PurchaseOrder>().AsNoTracking().Where(x => !x.IsDeleted);
