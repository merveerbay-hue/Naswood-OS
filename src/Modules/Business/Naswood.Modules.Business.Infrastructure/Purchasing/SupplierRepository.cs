...
public sealed class SupplierRepository : ISupplierRepository
{
    ...
    public SupplierRepository(BusinessDbContext db) => _db = db;

    public Task<Supplier?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default) =>
        _db.Set<Supplier>().FirstOrDefaultAsync(x => x.Id == id, cancellationToken);

    public async Task AddAsync(Supplier entity, CancellationToken cancellationToken = default) =>
        await _db.Set<Supplier>().AddAsync(entity, cancellationToken).ConfigureAwait(false);

    public async Task<(IReadOnlyList<Supplier> Items, int Total)> SearchAsync(string? q, int page, int pageSize, CancellationToken cancellationToken = default)
    {
        var query = _db.Set<Supplier>().AsNoTracking().Where(x => !x.IsDeleted);
