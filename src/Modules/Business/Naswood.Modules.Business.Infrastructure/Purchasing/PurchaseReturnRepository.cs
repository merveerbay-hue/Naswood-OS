...
public sealed class PurchaseReturnRepository : IPurchaseReturnRepository
{
    ...
    public PurchaseReturnRepository(BusinessDbContext db) => _db = db;

    public Task<PurchaseReturn?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default) =>
        _db.Set<PurchaseReturn>().FirstOrDefaultAsync(x => x.Id == id, cancellationToken);

    public async Task AddAsync(PurchaseReturn entity, CancellationToken cancellationToken = default) =>
        await _db.Set<PurchaseReturn>().AddAsync(entity, cancellationToken).ConfigureAwait(false);

    public async Task<(IReadOnlyList<PurchaseReturn> Items, int Total)> SearchAsync(string? q, int page, int pageSize, CancellationToken cancellationToken = default)
    {
        var query = _db.Set<PurchaseReturn>().AsNoTracking().Where(x => !x.IsDeleted);
