...
export function MaterialDefinitionDesigner() {
  ...
  const persistMutation = useMutation({
    mutationFn: async () => {
      const dimsLabel = `${def.thicknessMm}×${def.widthMm}×${def.lengthMm} mm`;
      const description = [def.species, dimsLabel, `Stock=${def.stockUom}`, `Grade=${def.grade}`, def.notes]
        .filter(Boolean)
        .join(' · ')
        .slice(0, 200);
      const definitionPayload = {
        ...def,
        NominalThicknessMm: Number(def.thicknessMm) || null,
        NominalWidthMm: Number(def.widthMm) || null,
        NominalLengthMm: Number(def.lengthMm) || null,
        dimsLabel,
      };
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="rounded-md border border-[var(--border-default)] bg-[var(--color-surface)] px-3 py-2 text-right">
            ...
            <p className="font-mono text-sm font-medium">
              {mintedCode
                ? `${mintedCode} · ${def.thicknessMm}×${def.widthMm}×${def.lengthMm} mm`
                : t('wizard.autoGenerated')}
            </p>
