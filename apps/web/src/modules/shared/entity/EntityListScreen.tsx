...
function isSystemIdField(field: EntityField): boolean {
  return field.systemId === true || SYSTEM_ID_KEYS.has(field.key);
}
...
export function EntityListScreen({
  ...
  const formFields = useMemo(() => fields.filter((f) => !isSystemIdField(f)), [fields]);
  const columns = useMemo(() => fields.slice(0, 6), [fields]);

  const [form, setForm] = useState<Record<string, string>>(() =>
    Object.fromEntries(
      fields
        .filter((f) => !isSystemIdField(f))
        .map((f) => [
          f.key,
          f.key === 'Status'
            ? route.includes('goods') || route.includes('transfer') || route.includes('count') || route.includes('adjust')
              ? 'Draft'
              : 'Active'
            : '',
        ]),
      ...
      {!readOnly && !isJobCta && showCreate ? (
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-[var(--text-muted)]">{t('wizard.systemCode')}: {t('wizard.autoGenerated')}</p>
          </CardHeader>
